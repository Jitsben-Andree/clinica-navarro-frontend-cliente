...
export class PortalComponent implements OnInit, OnDestroy {
  ...
  private cargarPerfil() {
    ...
    if (!usuarioId) {
      ...
      usuarioId = storedId ? Number(storedId) : null;
    }
    ...
    if (usuarioId && !isNaN(usuarioId)) {
      this.portalService.obtenerMiPerfil(usuarioId).subscribe({
        next: (perfil) => {
          if (perfil?.nombres) {
            const nombreCorto = perfil.nombres.split(' ')[0];
            this.pacienteNombre.set(nombreCorto);
          }
          this.ejecutarBienvenida();
        },
        error: (error) => {
          console.error('Error al mapear credenciales del paciente:', error);
          this.pacienteNombre.set('Paciente');
          this.ejecutarBienvenida();
        }
      });
    } else {
      console.warn('ID de usuario no resoluble en sesión activa');
  ...
  cerrarSesion() {
    if (confirm('¿Estás seguro de que deseas salir de tu portal de salud?')) {
      this.forzarCerrarSesion();
    }
  }
